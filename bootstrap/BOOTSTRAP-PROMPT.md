<!--
이 파일은 "부트스트랩 프롬프트"다.
사용 방법: 아래 ===== 줄 사이의 본문 전체를 복사해, /review-watch 하네스가
전혀 설치되지 않은 깨끗한 Claude Code 세션에 그대로 붙여넣어라.
에이전트가 7개 파일을 생성해 하네스를 구축한다.

검증 범위 주의 (작성자용 메모 — 붙여넣어도 무해):
이 프롬프트는 7개 파일 명세를 본문에 포함한 "이식형"이다. 따라서 이 테스트가
검증하는 것은 "프롬프트 1개로 하네스가 정확히 이식되는가"이지, "에이전트가
설계 원리만 보고 스스로 재구성하는가"가 아니다. 후자(자율 구축)는 명세를
주지 않는 별도 프롬프트로 따로 검증해야 한다. 이 한계를 알고 결과를 해석하라.
-->
