<!--
이 파일은 "자율 구축형 부트스트랩 프롬프트"다 (복붙형 BOOTSTRAP-PROMPT.md 와 별개).

복붙형과의 차이:
- 복붙형: 7파일 원문을 그대로 주고 "받아쓰라" → 재현 100%, 자율 0
- 자율형(이 파일): 7파일 원문을 절대 안 줌. 설계 원리·기능 계약·결함
  처방만 주고 "네가 7파일을 작성하라" → 자율 100%, 재현 편차 큼

검증 범위 (작성자 메모 — 붙여넣어도 무해):
이 프롬프트가 검증하는 것은 "에이전트가 설계 원리만으로 스스로 하네스를
재구성하는가"다. 결과물은 원본과 글자 단위로 다를 것이 정상이며, 그래서
1:1 diff가 아니라 "기능 계약 충족도"로 평가한다. 결과물이 계약을 어기면
(예: frontmatter 키 누락, patch형 아닌 재호출형 게이트, 환각 방어 없음)
자율 구축 실패다. 글자가 같은지가 아니라 계약을 지키는지가 합격선.

사용법: 아래 ===== 사이 본문을 /review-watch 가 설치 안 된 깨끗한 세션에
붙여넣어라.
-->
